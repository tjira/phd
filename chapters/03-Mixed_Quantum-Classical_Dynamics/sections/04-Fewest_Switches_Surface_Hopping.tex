...
The \acrshort{fssh} method seemingly provides us two different ways to calculate the population of the electronic states. One is the ensemble-averaged population obtained by counting the number of trajectories on each state, and the other is the quantum mechanical population obtained from the density matrix elements $\rho_{II}$. In an ideal scenario, these two populations should be equal. The \acrshort{tsh} method that satisfies this condition is called internally consistent. In practice, the \acrshort{fssh} method often fails to maintain this internal consistency, meaning that the ensemble-averaged population does not match the quantum mechanical population.\supercite{10.1063/1.474288,10.1063/1.1793991,10.1063/1.471791}
...
\end{equation}where $E_I$ and $E_J$ are the energies of states $I$ and $J$, $E_{\symrm{kin}}$ is the kinetic energy of the trajectory, and $C$ is an empirical parameter that can be adjusted to improve the agreement with quantum mechanical results. To ensure that the total population remains normalized after the transformation, we have to renormalize the current state coefficient as
...
To maintain balance within the \acrshort{fssh} ensemble, Tully proposed that during a frustrated hop, the electronic state remains unchanged, but the nuclear momentum component along the coupling vector is reversed ($\tilde{p}\to-\tilde{p}$), simulating an elastic bounce against the upper potential energy surface. While empirically successful, this momentum reversal is an \textit{ad-hoc} algorithmic prescription rather than a consequence of the equations of motion.

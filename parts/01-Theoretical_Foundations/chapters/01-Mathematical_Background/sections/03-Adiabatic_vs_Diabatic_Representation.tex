...
\end{equation}
where $T_{\symrm{n}}$ is the nuclear kinetic energy operator and $H_{\symrm{el}}$ is the electronic Hamiltonian, which we define here to implicitly include the nuclear-nuclear repulsion $V_{\symrm{NN}}(\symbf{R})$ in addition to the electronic kinetic energy and all Coulomb interactions. The nuclei are treated as fixed at positions $\symbf{R}$.
...
\begin{equation}
\i\frac{\partial}{\partial{t}}\ket{\tilde\chi_\alpha(\symbf{R},t)}=T_{\symrm{n}}\ket{\tilde\chi_\alpha(\symbf{R},t)}+\sum_\beta\tilde{V}_{\alpha\beta}(\symbf{R})\ket{\tilde\chi_\beta(\symbf{R},t)},
\end{equation}
where $T_{\symrm{n}}$ operator is the standard nuclear kinetic energy, which in this representation acts purely on the nuclear coefficients since the derivative couplings vanish. The $\tilde{V}$ term is equivalent to the electronic Hamiltonian in Eq.~\eqref{eq:boa_hamiltonian} evaluated in the diabatic basis. We refer to it here as the potential matrix.

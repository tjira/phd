\section{\filetotitle}\label{sec:adiabatic_vs_diabatic_representation}
...
For each fixed nuclear configuration $\symbf{R}$ we can solve the electronic \acrfull{tise}
\begin{equation}
...
Physically, the adiabatic representation has two key features. The electronic Hamiltonian is diagonal which means that each $E_I(\symbf{R})$ defines a \acrshort{pes} on which the nuclei move. And second, nonadiabatic effects appear as derivative couplings in the nuclear kinetic energy, encoded in $C_{IJ}(\symbf{R})$.

The adiabatic picture is particularly intuitive far from avoided crossings or \acrfull{ci}, where the nuclei move approximately on a single surface $E_I(\symbf{R})$. Near regions where surfaces approach or intersect, the derivative couplings become large and the adiabatic representation can become numerically inconvenient.
...
However, removing the derivative couplings in this way introduces off-diagonal elements into the potential energy matrix. The nuclear \acrshort{tdse} in the diabatic representation then has the form
\begin{equation}

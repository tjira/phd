...
\end{equation}
where $\ket{\delta\Psi}$ represents arbitrary variations within the tangent space. The Eq.~\eqref{eq:dfvp_orthogonality_condition} must hold for all $\ket{\delta\Psi}\in T_{\Psi}\mathcal{M}$ and constitutes the formal definition of the \gls{dfvp}, yielding \glspl{eom} for parameters $\symbf{\alpha}$.

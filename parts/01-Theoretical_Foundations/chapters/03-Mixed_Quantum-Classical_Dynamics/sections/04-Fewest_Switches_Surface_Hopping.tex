...
The \gls{fssh} method, developed by Tully in 1990,\supercite{10.1063/1.459170} is one of the most widely used \gls{tsh} algorithms. In \gls{fssh}, an ensemble of classical trajectories is propagated on adiabatic \gls{pes} according to Eq.~\eqref{eq:tsh_eom}, with stochastic hops between electronic states determined by the instantaneous electronic populations. The key idea is to minimize the number of hops while ensuring that the ensemble-averaged populations match the quantum mechanical populations.

Before we can obtain the hopping probabilities, it is convenient to express Eq.~\eqref{eq:tsh_eom} in terms of the density matrix elements $\rho_{IJ}=c_I c_J^\ast$ as
...
\end{equation}
where the diagonal elements $\rho_{II}=\lvert c_I\rvert^2$ represent the populations of the adiabatic states, and the off-diagonal elements $\rho_{IJ}$ ($I\neq J$) represent the coherences between states. It is also useful to note that the electronic Hamiltonian (potential) is Hermitian and the \gls{tdc} is anti-Hermitian. From Eq.~\eqref{eq:fssh_density_matrix_eom}, we can derive the time evolution of the populations $\rho_{II}$ using the properties of complex numbers as
...
P_{J\to I}=\frac{b_{JI}\Delta t}{\rho_{JJ}},
\end{equation}
...
To maintain balance within the \gls{fssh} ensemble, Tully proposed that during a frustrated hop, the electronic state remains unchanged, but the nuclear momentum component along the coupling vector is reversed ($\tilde{p}\to-\tilde{p}$), simulating an elastic bounce against the upper \gls{pes}. While empirically successful, this momentum reversal is an \textit{ad-hoc} algorithmic prescription rather than a consequence of the equations of motion.

...
\end{equation}
where $\symbf{M}$ is a diagonal matrix with masses for each coordinate, $\symbf{R}(t)$ is nuclear geometry at time $t$, and $E_I$ is the electronic energy of the state $I$ at the nuclear configuration $\symbf{R}(t)$. Meanwhile, the electrons evolve according to the \acrshort{tdse}, with the electronic wavefunction $\ket{\Phi(t)}$ expanded in the basis of instantaneous adiabatic eigenvectors of the electronic Hamiltonian $H_{\symrm{el}}$ as
\begin{equation}
...
\end{equation}
With $\symbf{c}(t)=\left(c_1(t),c_2(t),\dots,c_N(t)\right)^{\symrm{T}}$ the Eq.~\eqref{eq:sh_tdse_coefs} is a \acrshort{tdse} in a finite-dimensional Hilbert space $\mathbb{C}^N$ and can be rewritten as
\begin{equation}

...
Since we now know that the integrand of the remaining integral behaves as $\frac{1}{t}\frac{\ddot B}{B}\sim t^{-3}$ when $\lvert t\rvert\to\infty$, we can evaluate the integral using the same semicircular contour as before. The contribution from the semicircle vanishes, and we are left with the integral along the real axis. The integral is now equivalent to the residue of the integrand at $t=0$ times $2\pi\i$
\begin{equation}
